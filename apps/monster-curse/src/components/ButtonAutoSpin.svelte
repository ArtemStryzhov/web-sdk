<script lang="ts">
	...
	// Check if we're in bonus game (freegame)
	const isBonusGame = $derived(context.stateGame.gameType === 'freegame');
	
	const disabled = $derived.by(() => {
		// Disable autoplay in bonus game
		if (isBonusGame) return true;
	...
	<Button {...props} {sizes} {onpress} {disabled} anchor={0.5}>
		{#snippet children({ center, hovered, pressed })}
			...
			<Sprite
				{...center}
				anchor={0.5}
				key={disabled ? "autospin_mob_inactive" : (active ? "autospin_mob_inactive" : "autospin_mob_default")}
				width={sizes.width}
				height={sizes.height}
